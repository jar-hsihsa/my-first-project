...
def parse_expense_node(ctx: Context, node_input: Any) -> Event:
    ...
    raise ValueError(f"Could not parse input as JSON and no cached expense exists: {node_input}")
...
def auto_approve_node(node_input: dict) -> dict:
    ...
    return {
        "expense": expense,
        "decision": "Approved",
        "reason": f"Amount ${expense['amount']} is under threshold of ${EXPENSE_THRESHOLD}. Auto-approved.",
...
async def risk_review_node(ctx: Context, node_input: dict) -> Event:
    ...
    prompt = f"""
    You are an AI risk analyst reviewing corporate expense reports.
