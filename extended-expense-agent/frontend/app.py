...
def process_events(events):
    ...
    if not paused and final_output:
        ...
        if isinstance(final_output, dict) and "expense" in final_output:
            save_expense(final_output["expense"], final_output.get("decision", "Approved"))
...
with st.sidebar:
    ...
    if st.session_state.role == "Admin":
        pending_count = 1 if st.session_state.waiting_for_input else 0
        badge = f'<span class="nav-badge">{pending_count}</span>' if pending_count else ""
        st.markdown(f'<div class="nav-item">📊  Dashboard</div>', unsafe_allow_html=True)
        st.markdown(
        ...
        st.markdown('<div class="nav-item">👥  Users</div>', unsafe_allow_html=True)
    else:
        st.markdown('<div class="nav-active">📊  Dashboard</div>', unsafe_allow_html=True)
        st.markdown('<div class="nav-item">➕  Submit Expense</div>', unsafe_allow_html=True)
        st.markdown('<div class="nav-item">📁  My Expenses</div>', unsafe_allow_html=True)
...
if st.session_state.role == "Employee":
    st.markdown(
        '<div class="breadcrumb"><a href="#">Dashboard</a> &gt; Submit Expense</div>',
        unsafe_allow_html=True,
    )

    # ── My Expense History ───────────────────────────────────
    expenses = get_employee_expenses(st.session_state.email)
    if expenses:
        st.markdown(
            f'<div class="section-title">My Expenses ({len(expenses)})</div>',
            unsafe_allow_html=True,
        )
        ...
        for exp in expenses:
            ...
            emp_initials = _initials(exp.get("submitter", ""))
            ...
            rows_html += f"""<tr>
                ...
                <td>
                    <div class="emp-chip">
                        <span class="emp-avatar">{emp_initials}</span>
                        <div>
                            <div class="emp-name">{_display_name(exp.get('submitter',''))}</div>
                        </div>
                    </div>
                </td>
                <td>{exp.get('date','—')}</td>
    ...
    with tab_receipt:
        ...
        with col_d1:
            if not has_demo:
                ...
                if st.button("🗑️  Remove Demo Receipt"):
        ...
        if uploaded_file is not None:
            ...
            st.image(image_to_submit, caption="Uploaded Receipt", width=300)
        elif (
            ...
            st.image(image_to_submit, caption="Demo Receipt", width=300)
    ...
    if st.session_state.waiting_for_input:
        st.warning("⏳  Expense submitted — pending Admin approval.")
    ...
    if not st.session_state.waiting_for_input and st.session_state.final_output:
        ...
        if isinstance(out, dict):
            ...
            status_cls = "status-approved" if decision == "Approved" else "status-rejected"
            st.markdown(
...
# ╚═══════════════════════════════════════════════════════════╝
    ...
    st.markdown(
        '<div class="breadcrumb"><a href="#">Dashboard</a> &gt; Pending Approvals</div>',
        unsafe_allow_html=True,
    )
    ...
    if st.session_state.waiting_for_input:
        details = parse_interrupt_details(st.session_state.interrupt_message)
        submitter_str = details.get("submitter", "Unknown")
        ...
        exp_id = "EX" + str(abs(hash(st.session_state.session_id)) % 9999).zfill(4)
        ...
        st.markdown(
            f"""<div class="detail-card">
                ...
                <div class="detail-grid">
                    ...
                    <div class="detail-value">{date.today().strftime('%d %b %Y')}</div>
                    <div class="detail-label">Amount:</div>
                    ...
                    <div class="detail-value" style="color:#D97706;">{risk_str}</div>
                </div>
            </div>""",
            unsafe_allow_html=True,
        )
        ...
        if st.session_state.submitted_receipt_image:
            with st.expander("🧾  Invoice Attachment — View Receipt", expanded=True):
                try:
                    img_bytes = base64.b64decode(
                        st.session_state.submitted_receipt_image
                    )
                    st.image(
                        img_bytes,
                            ...
                            "parts": [
                                {
                                    "function_response": {
                                        ...
                                        "response": {
                                            "output": f"Reject: {rejection_reason.strip()}"
                                        },
                                    }
                                }
                            ],
    ...
    if all_expenses:
        st.markdown(
            f'<div class="section-title" style="margin-top:1rem;">All Expenses ({len(all_expenses)})</div>',
            unsafe_allow_html=True,
        )
